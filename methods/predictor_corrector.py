"""AB4-AM3 predictor-corrector method with RK4 startup."""
...
def solve(
    ...
    if len(t) < 5:
        raise ValueError("Predictor-Corrector requires RK4 startup points.")
    ...
    for i in range(3, len(t) - 1):
        ...
        f_predictor = f(t[i + 1], predictor)
        function_evaluations += 1

        w[i + 1] = w[i] + h * (9.0 * f_predictor + 19.0 * f_i - 5.0 * f_i_1 + f_i_2) / 24.0

        if i + 1 < len(t) - 1:
            f_values[i + 1] = f(t[i + 1], w[i + 1])
            function_evaluations += 1

    return make_result("Predictor-Corrector", h, t, w, function_evaluations)
